...
function getOneOrder(req, res, next) {
  ...
  ordersModel.getOneOrder(req.params.orderId).then(data => {
...
function getAllOrders(req, res, next) {
  ...
  ordersModel.getAllOrders(req.params.shopId).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
...
function updateOrder(req, res, next) {
  ...
  ordersModel.updateOrder(req.params.orderId, req.body).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
...
function updateOrderSupply(req, res, next) {
  ...
  ordersModel.updateOrderSupply(req.params.orderId, req.body).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
