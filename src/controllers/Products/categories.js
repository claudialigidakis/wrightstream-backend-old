...
function getOneCategory(req, res, next) {
  ...
  categoiesModel.getOneCategory(req.params.categoryId).then(data => {
...
function getAllCategories(req, res, next) {
  ...
  categoiesModel.getAllCategories(req.params.shopId).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
...
function createCategories(req, res, next) {
  ...
  categoiesModel.createCategories(req.body.name, parseInt(req.params.shopId)).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
...
function updateCategories(req, res, next) {
  ...
  categoiesModel.updateCategories(req.params.categoryId, req.body).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
...
function removeCategories(req, res, next) {
  ...
  categoiesModel.removeCategories(parseInt(req.params.categoryId)).then(function(data) {
    res.status(200).send({data})
  }).catch(next)
}
