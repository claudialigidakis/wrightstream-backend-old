...
function getOneSupply(req, res, next) {
  ...
  suppliesModel.getOneSupply(req.params.suppliesId).then(data => {
...
function getAllSupplies(req, res, next) {
  ...
  suppliesModel.getAllSupplies(req.params.shopId).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
...
function removeSupplies(req, res, next) {
  ...
  suppliesModel.removeSupplies(parseInt(req.params.suppliesId)).then(function(data) {
    res.status(200).send({data})
  }).catch(next)
}
...
function updateSupplies(req, res, next) {
  ...
  suppliesModel.updateSupplies(req.params.suppliesId, req.body.name, req.body.stock_qty, req.body.measure_type, req.body.stock_qty_measure_type, req.body.source_id, req.body.kind_id).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
