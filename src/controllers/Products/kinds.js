...
function getOneKind(req, res, next) {
  ...
  kindsModel.getOneKind(req.params.kindId).then(data => {
...
function getAllKinds(req, res, next) {
  ...
  kindsModel.getAllKinds(req.params.shopId).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
...
function removeKinds(req, res, next) {
  ...
  kindsModel.removeKinds(parseInt(req.params.kindId)).then(function(data) {
    res.status(200).send({data})
  }).catch(next)
}
...
function updateKinds(req, res, next) {
  ...
  kindsModel.updateKinds(req.params.kindId, req.body).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
