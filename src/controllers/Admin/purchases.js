...
function purchaseStatuses(req, res, next) {
  ...
  adminModel.purchaseStatuses(req.params.shopId).then(data => {
...
function newPurchases(req, res, next) {
  ...
  adminModel.newPurchases(req.params.shopId).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
...
function productionPurchases(req, res, next) {
  ...
  adminModel.productionPurchases(req.params.shopId).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
...
function totalPurchases(req, res, next) {
  ...
  adminModel.totalPurchases(req.params.shopId).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
...
function completedPurchases(req, res, next) {
  ...
  adminModel.completedPurchases(req.params.shopId).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
...
function purchasesHistory(req, res, next) {
  ...
  adminModel.purchasesHistory(req.params.shopId).then(data => {
    res.status(200).send({data})
  }).catch(next)
}
