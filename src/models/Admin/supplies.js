...
function mostUsed(shopId) {
  return knex('items').where({shop_id: shopId, archived: false}).select('id', 'name').then(items => {
    const promise = items.map(item => {
      return knex('items_supplies').innerJoin('supplies', 'supplies.id', 'items_supplies.supplies_id').where({'items_supplies.item_id': item.id}).select('qty', 'qty_measure', 'supplies_id', 'name', 'measure_type').then(supply => {
...
function mostOrdered(shopId) {
  return knex('purchases').where({shop_id: shopId}).select('id', 'purchase_date').then(purchases => {
