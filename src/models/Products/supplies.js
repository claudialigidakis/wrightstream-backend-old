...
function getOneSupply(supplyId) {
  return (knex('supplies').where({id: supplyId}).first())
}

function getAllSupplies(shopId) {
  return (knex('supplies').where({shop_id: shopId}))
}
...
function createSupplies(body, shopId) {
  ...
  return (knex('supplies').insert({
    name: body.name,
    stock_qty: stock,
    stock_qty_measure_type: body.stock_measure_type,
...
function removeSupplies(supplyId) {
  return (knex('supplies').where({id: supplyId}).del())
}
...
function updateSupplies(supplyId, name, stock_qty, measure_type, stock_qty_measure_type, source_id, kind_id) {
  ...
  name
    ? toUpdate.name = name
    : null
  stock_qty_measure_type
    ? toUpdate.stock_qty_measure_type = stock_qty_measure_type
    : null
  measure_type
  ...
  return (knex('supplies').update(toUpdate).where({id: supplyId}).returning('*'))
}
