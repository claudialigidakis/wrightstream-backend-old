...
function createBacklog(shopId, purchase_ids) {
  return knex('purchases').where({shop_id: shopId}).andWhere('purchases.inventory', false).then(purchases => {
    const promise = purchases.map(purchase => {
      return knex('purchases_supplies').join('supplies', 'supplies.id', 'purchases_supplies.supplies_id').where('purchases_supplies.purchase_id', purchase.id).andWhere('purchases_supplies.completed', false).select('supplies_id', 'supplies_qty', 'supplies_measurement', 'completed', 'measure_type').then(supply => {
