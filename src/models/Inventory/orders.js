...
function getAllOrders(shopId) {
  return knex('orders').where({shop_id: shopId}).then(orders => {
    const promises = orders.map(order => {
      return knex('orders_supplies').join('supplies', 'supplies.id', 'orders_supplies.supply_id').select('orders_supplies.supply_id', 'orders_supplies.supply_qty', 'orders_supplies.supply_measure_type', 'orders_supplies.supply_status', 'supplies.source_id').orderBy('supplies.source_id', 'asc').where('orders_supplies.order_id', order.id).then(supplies => {
        supplies.map(supply => {
...
function createOrder(shopId, body) {
  return knex('orders').insert({shop_id: shopId}).returning('*').then(order => {
    if (body.order) {
      return helperModel.orderPredictor(body.order).then(details => {
        const detailMap = details.map(ele => {
          return (knex('orders_supplies').insert({order_id: order[0].id, supply_id: ele.supply_id, supply_qty: ele.supply_qty, supply_measure_type: ele.supply_measure_type}).returning('*'))
        })
...
function updateOrder(orderId, body) {
  const toUpdate = {}
  body.status_id
    ? toUpdate.status_id = body.status_id
    : null
  return (knex('orders').update(toUpdate).where({id: orderId}).returning('*'))
}
...
function updateOrderSupply(orderId, body) {
  ...
  body.supply_qty || body.supply_qty === 0
    ...
    : null
  body.supply_measure_type
    ? toUpdate.supply_measure_type = body.supply_measure_type
    : null
  body.supply_status
    ? toUpdate.supply_status = body.supply_status
    : null
  return (knex('orders_supplies').update(toUpdate).where({order_id: orderId, supply_id: body.supply_id}).returning('*')).catch(err => {
    return err
  }).then(supply => {
...
function addInventory(newSupply) {
  const newUpdate = {}
  return (knex('supplies').where({id: newSupply[0].supply_id}).returning('*')).then(supply => {
    ...
    return (knex('supplies').update(newUpdate).where({id: newSupply[0].supply_id}).returning('*'))
  })
}
